...
int s21_from_float_to_decimal(float src, s21_decimal *dst) { // rounding may be false, but I don't remember in which case :)
    ...
    strncpy(significant_digits + strlen(significant_digits), dot_adress + 1, 7 - copy_digits_amount);
...
int s21_from_decimal_to_float(s21_decimal src, float *dst) {
    ...
    long double tmp = (pow(2, 64) * src.bits[2] + pow(2, 32) * src.bits[1] + src.bits[0]) / divisor;
